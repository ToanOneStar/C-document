...
## 3.2. Lỗi trang (Page Faults) và Xử lý lỗi trang

**Lỗi trang** là ngoại lệ MMU tạo ra khi tiến trình truy cập dữ liệu ảo nhưng dữ liệu không có trong RAM.9 Đây là hoạt động bình thường của bộ nhớ ảo, cho phép tải dữ liệu theo yêu cầu (demand paging).
...
**Xử lý lỗi trang:** CPU tạo ngắt khi truy cập trang không có trong RAM. OS chặn tiến trình, tìm trang trên đĩa, đưa vào RAM (dùng thuật toán thay thế trang nếu cần). Bảng trang được cập nhật. CPU tiếp tục thực thi. Thời gian xử lý là Thời gian phục vụ lỗi trang. Hiệu suất tổng thể được tính bằng Thời gian truy cập hiệu quả (EAT):

EAT = (1-p) * Memory Access Time + p * Page fault time, với p là tỷ lệ lỗi trang.

Lỗi trang là bình thường, nhưng nếu quá thường xuyên sẽ giảm hiệu suất. EAT cho thấy tỷ lệ lỗi trang cao làm tăng thời gian truy cập bộ nhớ hiệu quả. Cần thuật toán thay thế trang và chiến lược tối ưu hóa để giảm lỗi trang, tránh lãng phí CPU vào hoán đổi. Quản lý lỗi trang là yếu tố then chốt quyết định hiệu suất hệ thống, đòi hỏi tinh chỉnh liên tục.

## 3.3. Các thuật toán thay thế trang (Page Replacement Algorithms)

Khi lỗi trang xảy ra và không có khung trang trống, OS phải quyết định trang nào trong RAM sẽ bị hoán đổi ra đĩa. Mục tiêu là giảm thiểu lỗi trang. "Chuỗi tham chiếu" được dùng để đánh giá hiệu quả thuật toán.

**Các thuật toán thay thế trang phổ biến:**
- **FIFO:** Thay thế trang cũ nhất.
- **Optimal (OPT/MIN):** Tỷ lệ lỗi trang thấp nhất, thay thế trang không dùng lâu nhất trong tương lai. Lý thuyết, không thực tế vì cần biết trước tương lai.
...
Các thuật toán này cho thấy đánh đổi giữa thực tiễn và hiệu suất. OPT có tỷ lệ lỗi trang thấp nhất nhưng không thực tế. FIFO và LRU thực tế hơn. LRU hiệu quả hơn FIFO vì xấp xỉ OPT. Thiết kế hệ thống luôn đánh đổi giữa hiệu suất lý tưởng và khả thi. OPT là chuẩn mực lý thuyết để đánh giá thuật toán thực tế, thúc đẩy phát triển thuật toán thay thế trang thông minh hơn.

## 3.4. Hiện tượng Thrashing

**Thrashing** là tình trạng hệ thống dành quá nhiều thời gian hoán đổi trang thay vì thực thi tiến trình. Thiếu RAM gây lỗi trang thường xuyên, buộc OS hoán đổi liên tục. Hoạt động này tiêu tốn CPU và I/O, giảm hiệu suất, dẫn đến nhiều lỗi trang hơn.
...
**Dấu hiệu thrashing:**
1. **CPU cao, hiệu suất thấp:** CPU dành nhiều thời gian xử lý lỗi trang và hoán đổi.
2. **Hoạt động đĩa tăng:** Ổ đĩa liên tục đọc/ghi trang hoán đổi.
3. **Tỷ lệ lỗi trang cao:** Lỗi trang tăng vọt, tiến trình liên tục truy cập trang không có trong RAM.
...
- **Chính sách thay thế trang hiệu quả:** Chọn thuật toán tốt hơn (ví dụ: LRU) để giảm lỗi trang.
...
# 4. Bộ nhớ ảo trong Lập trình C
## 4.1. Cấu trúc bộ nhớ của chương trình C
...
## 4.2. Phân mảnh bộ nhớ (Memory Fragmentation)
...
# 5. Tương tác Nâng cao giữa C và Bộ nhớ ảo: Ánh xạ tệp vào bộ nhớ (mmap)
...
## 5.1. Khái niệm và Ưu điểm của mmap()
...
## 5.2. Cách sử dụng mmap() để xử lý các tệp lớn và cấp phát bộ nhớ vượt quá RAM vật lý
...
```
## 5.3. So sánh mmap() với I/O truyền thống (read() / write())
...
Bộ nhớ ảo là một kỹ thuật quản lý bộ nhớ không thể thiếu, là nền tảng cho hiệu suất, bảo mật và khả năng mở rộng của các hệ thống máy tính hiện đại. Nó tạo ra ảo ảnh về một không gian bộ nhớ lớn hơn nhiều so với RAM vật lý có sẵn, cho phép các chương trình lớn chạy hiệu quả và hỗ trợ đa nhiệm mạnh mẽ thông qua cơ chế cô lập bộ nhớ. Các khái niệm cốt lõi như địa chỉ ảo, địa chỉ vật lý, MMU, phân trang, khung trang và bảng trang là những trụ cột kỹ thuật cho phép sự trừu tượng hóa này. Hệ điều hành đóng vai trò trung tâm trong việc quản lý bộ nhớ ảo, xử lý các lỗi trang, hoán đổi dữ liệu và áp dụng các thuật toán thay thế trang để tối ưu hóa việc sử dụng tài nguyên.

Trong bối cảnh lập trình C, bộ nhớ ảo định hình cấu trúc bộ nhớ của chương trình (Text, Data, Heap, Stack) và là cơ sở cho các cơ chế cấp phát bộ nhớ động như malloc(), calloc(), realloc(), và free(). Sự tương tác giữa các hàm thư viện C này và các lệnh gọi hệ thống của kernel là một ví dụ về cách các lớp trừu tượng được xây dựng để tối ưu hóa hiệu suất. Các kỹ thuật nâng cao như ánh xạ tệp vào bộ nhớ (mmap()) cho phép các chương trình C tương tác trực tiếp với bộ nhớ ảo để xử lý các tệp lớn một cách hiệu quả, vượt trội so với I/O truyền thống trong nhiều trường hợp.

Tuy nhiên, bộ nhớ ảo không phải là không có thách thức. Hiệu suất chậm hơn do truy cập đĩa, chi phí hoạt động của hệ điều hành, và nguy cơ thrashing là những nhược điểm cần được quản lý cẩn thận. Hiện tượng thrashing, một vòng lặp phản hồi tiêu cực của việc hoán đổi trang quá mức, là một lời nhắc nhở rõ ràng về giới hạn của bộ nhớ ảo khi không có đủ tài nguyên vật lý.
